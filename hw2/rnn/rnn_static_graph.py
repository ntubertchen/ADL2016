...
class Config(object):
  ...
  model_name = "rnn_embed=5_l2=0.050000_lr=0.100000.weights"
  ...
  def run_epoch(self, new_model=False, verbose=True):
    ...
    with tf.Session() as sess:
      ...
      for step, tree in enumerate(self.train_data):
        feed_dict = self.build_feed_dict(tree.root)
        loss_value, _ = sess.run([self.root_loss, self.train_op],
                                 feed_dict=feed_dict)
...
#  plot_loss_history(stats)
  ...
  predictions_path = sys.argv[1] + "answer.txt"
