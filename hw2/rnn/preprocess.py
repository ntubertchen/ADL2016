...
def demo():
    ...
    count = 0
    sentence = ""
    treepath = sys.argv[1] + sys.argv[2]
    with open(treepath, 'r') as file_in:
        for line in file_in:
            left += line.count("(")
            right += line.count(")")
            sentence += line
            ...
            if left is right and left > 0:
                ...
                treetransforms.collapse_unary(collapsedTree,collapsePOS=True, collapseRoot = True)

                # convert the tree to CNF
                cnfTree = deepcopy(collapsedTree)
                treetransforms.chomsky_normal_form(cnfTree)

                sentence2 = cnfTree.pprint()
                print(sentence2)
                sentence = ""
