...
class Tree:
    ...
    def __init__(self, treeString, feature,openChar='(', closeChar=')'):
        tokens = []
    ...
    def parse(self, tokens, parent=None):
        ...
        # New node
        if self.feature is -1:
            node = Node(int(random.randint(0,1)))
        else:
            node = Node(int(self.feature))  # zero index labels
        ...
        if split + 1 is len(tokens):
            node.right = self.parse(tokens[2:split], parent=node)
        else:
...
def loadTrees(dataSet='train',feature='0'):
    """
    Loads training trees. Maps leaf node words to word ids.
    """
    file = './rnn/trees/%s.txt' % dataSet
    print "Loading %s trees.." % dataSet
    with open(file, 'r') as fid:
        trees = [Tree(l,feature) for l in fid.readlines()]
...
def simplified_data(num_train, num_dev, num_test):
    ...
    random.seed(0)
    trees = loadTrees('pos_line',1) + loadTrees('neg_line',0) 
    trees_test = loadTrees('test_line',-1)
    test_trees = [t for t in trees_test if t.root.label==0]
