...
tf.app.flags.DEFINE_string("answer_file", "", "answer file path")
...
def train():
  ...
  with tf.Session() as sess:
    ...
    def run_valid_test(data_set, mode): # mode: Eval, Test
        ...
        answer_file = FLAGS.answer_file
        print (answer_file)
        if task['intent'] == 1:
          f = open(answer_file,"w")
          for i in range(len(hyp_label_list)):
            f.write(hyp_label_list[i] + "\n")
          f.close()
        ...
        if task['tagging'] == 1:
          ...
          f = open(answer_file,"w")
          for i in range(len(hyp_tag_list)):
            for j in range(len(hyp_tag_list[i])):
              if j is 0:
                f.write("O ")
                f.write(hyp_tag_list[i][j])
              else:
                f.write(" ")
                f.write(hyp_tag_list[i][j])
            f.write("\n")
        ...
        # valid
        # valid_accuracy, valid_tagging_result = run_valid_test(dev_set, 'Eval',current_step,best_valid_score)        
