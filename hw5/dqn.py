...
class DeepQNetwork:
    def __init__(self, numActions, baseDir, args,se):
    ...
    def buildNetwork(self, name, trainable, numActions):
        ...
        with tf.variable_scope("fc2_" + name):
            #512 something and wind up with 4 action, decide which one to use
            W_fc2, b_fc2 = self.makeLayerVariables([512, numActions], trainable, "fc2")
            #y much like which action to use 
    ...
    def train(self, batch, stepNumber):
        #training via replay memory, espcially those whom ended up zero or terminal
        ...
        for i in range(0, len(batch)):
            #a=1 stands for which step took which action
